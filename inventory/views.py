...
def sales_list(request):
    ...
    if action == "import_ml_sales_xlsx":
        ...
        for idx, row in enumerate(rows, start=1):
            ...
            entry = orders.setdefault(
                order_id,
                {
                    ...
                    "iibb": Decimal("0.00"),
                    "items": [],
                    "ml_order_id": row.get("order_id") or "",
                },
            )
            entry["total"] += price_total
            entry["commission"] += commission
        ...
        skipped = 0
        for order_id, data in orders.items():
            reference = f"XLSX ORDER {order_id}"
            if Sale.objects.filter(reference=reference).exists():
                skipped += 1
                continue
            if not data["items"]:
            ...
            with transaction.atomic():
                sale = Sale.objects.create(
                    ...
                    total=data["total"].quantize(Decimal("0.01")),
                    reference=reference,
                    ml_order_id=str(data.get("ml_order_id") or ""),
                    ml_commission_total=data["commission"].quantize(Decimal("0.01")),
                    ml_tax_total=data["iibb"].quantize(Decimal("0.01")),
                    user=request.user,
                )
...
def _read_ml_sales_xlsx_rows(upload) -> tuple[list[dict], str | None]:
    ...
    price_unit_idx = _pick_index(["precio unitario", "precio unitario venta", "precio unitario bruto"])
    commission_idx = _pick_index(["comision", "comisión"])
    iibb_idx = _pick_index(["iibb", "impuestos", "impuesto", "iibb %"])
    date_idx = _pick_index(["fecha", "fecha venta", "fecha orden"])
    order_idx = _pick_index(["idventa", "id venta", "idorder", "id order", "orden", "operacion", "operación"])

    required = [product_idx, qty_idx, commission_idx, iibb_idx]
    if price_total_idx is None and price_unit_idx is None:
        required.append(None)
    if any(idx is None for idx in required):
        return [], "Faltan columnas obligatorias: Producto, Cantidad, Precio Bruto Venta (o Precio Unitario), Comisión e IIBB."
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        if qty <= 0:
            continue
        unit_price = _parse_decimal(row[price_unit_idx]) if price_unit_idx is not None else Decimal("0.00")
        price_total = _parse_decimal(row[price_total_idx]) if price_total_idx is not None else Decimal("0.00")
        if unit_price > 0 and price_total == 0:
            price_total = (unit_price * qty).quantize(Decimal("0.01"))
        commission = _parse_decimal(row[commission_idx])
        iibb = _parse_decimal(row[iibb_idx])
        created_at = row[date_idx] if date_idx is not None else None
        order_id_raw = row[order_idx] if order_idx is not None else None
        rows.append(
            {
                "title": title,
                "quantity": qty,
